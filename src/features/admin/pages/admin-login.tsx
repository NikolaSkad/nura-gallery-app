...
import type { AdminUser, VerifyOtpResponse } from '@/lib/auth/types';
...
export function AdminLogin() {
	...
	const handleVerifySuccess = async (response: VerifyOtpResponse) => {
		// Verify response carries only tokens — fetch the user from /auth/me
		// with the fresh access token so we can role-check BEFORE committing
		// auth state. Tokens stay out of localStorage until role is confirmed.
		let user: AdminUser;
		try {
			user = await bearerFetch<AdminUser>('/auth/me', response.accessToken.token);
		} catch {
		...
		if (user.role !== 'ADMIN') {
			toast.error('You must log in as an admin');
			setPhase('request');
			return;
		}
