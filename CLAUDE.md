...
12. **All card-like surfaces use `<Card>` from `@/components/ui/card` — not a raw `<div>` with `bg-*` + `rounded-*` + `p-*` classes.** Three variants (`default` solid dark, `glass` translucent, `primary` solid orange), four paddings (`none` / `sm` / `md` / `lg`), four radii (`none` / `sm` / `md` / `lg`). For clickable cards use `asChild` and wrap a `<Link>` or `<button>` so the whole surface is the hit target (`<Card asChild><Link …>`). Sub-components: `<CardTitle>`, `<CardDescription>`, `<CardContent>`. If a screen needs a one-off bg+radius that doesn't match any variant, add a variant — don't sprinkle utility classes that drift from the system.
...
- **Every mutation must invalidate (or directly update) every query whose data it changes.** Before writing the mutation, list the read endpoints that touch the same resource — list, detail, any nested counts — and call `queryClient.invalidateQueries({ queryKey: [...] })` for each in `onSuccess`. If a related read endpoint doesn't exist yet, leave the invalidation gap visible (don't pretend it's done) and wire it in the same PR as the new read hook. Stale lists after a create/update/delete are the #1 cache bug — assume one exists unless you've explicitly checked.
